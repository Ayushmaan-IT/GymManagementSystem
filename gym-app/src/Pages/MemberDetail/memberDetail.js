...
const MemberDetail = () => {
  ...
  const handleRenewSaveBtn = async () => {
    await axios
      ...
      .then((response) => {
        setData(response.data.member);
        toast.success(response.data.member);
      })
      .catch((err) => {
        toast.error("Something Went Wrong");
        console.log(err);
      ...
      <div className="mt-10 p-2">
        <div className="w-[100%] h-fit flex">
          ...
          <div className="w-2/3 mt-5 text-xl p-5">
            ...
            {isDateInPast(data?.nextBillDate) && (
              <div
                ...
                className={`mt-1 rounded-lg p-3 border-2 border-slate-900 text-center ${
                  renew && status === "Active"
                    ? "bg-gradient-to-r from-[#dc2626] via-[#f97316]  to-[#1f2937]"
                    : null
                }  w-full md:w-1/2 cursor-pointer hover:text-white hover:bg-gradient-to-r from-[#fde68a]  via-[#dc2626] to-[#111827]`}
              >
                Renew
              </div>
            )}
            ...
            {renew && status === "Active" ? (
              <div className="rounded-lg p-3 mt-5 h-fit bg-slate-50 w-[100%]">
                <div className="'w-full">
                  <div className="my-5">
                    ...
                    <div
                      className={`mt-3 rounded-lg p-3 border-2 border-slate-900 text-center w-1/2 mx-auto cursor-pointer hover:text-white hover:bg-gradient-to-r  from-[#fb923c] via-[#dc2626] to-[#111827]`}
                      onClick={() => {
                        handleRenewSaveBtn();
                      }}
                    >
