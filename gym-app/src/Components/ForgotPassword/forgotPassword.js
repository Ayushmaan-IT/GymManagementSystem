...
const ForgotPassword = () => {
  ...
  const verifyOTP = async () => {
    setLoader(true);
    await axios
      .post("http://localhost:4000/auth/reset-passwor/checkOtp", {
        email: inputField.email,
        otp: inputField.otp,
      })
      .then((response) => {
  ...
  return (
    <div className="w-full">
      ...
      <div
        className="bg-red-600 text-white mx-auto w-2/3 p-3 rounded-lg text-center font-semibold cursor-pointer border-2 border-red-700 hover:bg-white hover:text-red-700 transition-all duration-300"
