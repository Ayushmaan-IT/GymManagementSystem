...
exports.registerMember = async (req, res) => {
  try {
    ...
    // ✅ Check before using memberShip.months
    if (!memberShip) {
...
exports.monthlyMember = async (req, res) => {
  try {
    ...
    //  Get the first day of the current month (e.g., 2025-11-30 00:00:00)
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);

    // Get the last day of the current month (e.g., 2025-09-30 23:59:59)
...
exports.expiringWithin2Days = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: member.length
        ? "Fetched Members SuccessFully"
        : "No Such Member is Expiring within 2 Days. ",
      members: member,
      totalMembers: member.length,
    });
  } catch (err) {
...
exports.expiringWithInAWeek = async (req, res) => {
  try {
    const today = new Date();
    ...
    res.status(200).json({
      message: member.length
        ? "Fetched Members SuccessFully"
        : "No Such Member is Expiring within a week.",
      members: member,
      totalMembers: member.length,
    });
  } catch (err) {
